...
const loadSavedGlitchSettings = (): GlitchSettings => {
    try {
        ...
        if (saved) return { ...defaultGlitchSettings, ...JSON.parse(saved) };
    } catch (e) { console.warn('Failed to load glitch settings:', e); }
...
const loadSavedCyberRgbSettings = (): CyberRgbSettings => {
    try {
        ...
        if (saved) return { ...defaultCyberRgbSettings, ...JSON.parse(saved) };
    } catch (e) { console.warn('Failed to load cyber RGB settings:', e); }
...
const loadSavedTransitionGlitchSettings = (): TransitionGlitchSettings => {
    try {
        ...
        if (saved) return { ...defaultTransitionGlitchSettings, ...JSON.parse(saved) };
    } catch (e) { console.warn('Failed to load transition glitch settings:', e); }
