...
const client = createClient({
    projectId: process.env.VITE_SANITY_PROJECT_ID || 'argneoi8',
...
const migrate = async () => {
    ...
    // Fallback token check from default sanity config if not in env
    // but sanity exec handles auth context usually.
    // We use the client created by createClient, but if token is missing it won't write.
    // However, when running with `sanity exec`, we can utilize the `sanity` client context?
    // Let's rely on SANITY_AUTH_TOKEN first. If missing, we warn.
    // Actually `sanity exec` provides a pre-configured client? No, it just sets up environment.

    for (const proj of projects) {
        const detail = projectDetails[proj.id];
        console.log(`\n📦 Processing: ${proj.title} (${proj.id})`);

        const contentModules = [];

        if (detail) {
            // 1. Hook (-> Highlight or Default)
            if (detail.sections?.hook) {
                contentModules.push({
                    ...
                    content: [
                        {
                            _type: 'block',
                            children: [{ _type: 'span', text: detail.sections.hook.content }]
                        }
                    ]
                });
            }
            ...
            if (detail.sections?.context) {
                contentModules.push({
                    ...
                    content: [
                        ...
                        {
                            _type: 'block',
                            children: [{ _type: 'span', text: detail.sections.context.challenge }]
                        },
                        {
                            _type: 'block',
                            style: 'h4',
                            children: [{ _type: 'span', text: 'Background' }]
                        },
                        {
                            _type: 'block',
                            children: [{ _type: 'span', text: detail.sections.context.background }]
                        }
                    ]
                });
            }
                    ...
                    content: [
                        {
                            _type: 'block',
                            style: 'h4',
                            children: [{ _type: 'span', text: detail.sections.strategy.approach }]
                        },
                        ...detail.sections.strategy.insights.map((insight: string) => ({
                            _type: 'block',
                            style: 'normal',
                            listItem: 'bullet',
                            children: [{ _type: 'span', text: insight }]
                        }))
                    ]
                });
            }
            ...
            if (detail.sections?.outcome) {
                contentModules.push({
                    ...
                    content: [
                        {
                            _type: 'block',
                            children: [{ _type: 'span', text: detail.sections.outcome.reflection }]
                        },
                        ...detail.sections.outcome.results.map((res: string) => ({
                            _type: 'block',
                            style: 'normal',
                            listItem: 'bullet',
                            children: [{ _type: 'span', text: res }]
                        }))
                    ]
                });
            }
        ...
        const doc = {
            ...
            // Ensure detail exists before accessing fields
            coreMetrics: detail ? detail.coreMetrics.map((m: any) => ({
                _key: m.label,
                label: m.label,
                value: m.value,
                unit: m.unit
